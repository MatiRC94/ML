...
int PTEST;          /* cantidad de patrones de TEST (a99420rchivo .test) */
...
double **max_val;                /* valores maximos para limitar bins */
double **min_val;              /* valores minimos para limitar bins */
...
int define_matrix(){
  ...
  prob_d = (double*)calloc(N_Class,sizeof(double)) ;
  
  max_val = (double **)calloc(N_Class,sizeof(double *));
  min_val = (double **)calloc(N_Class,sizeof(double *));
  for (i=0 ; i< N_IN ; i++){
    max_val[i] = (double *)calloc(N_IN,sizeof(double));
    min_val[i] = (double *)calloc(N_IN,sizeof(double));
/*    for(j=0; j<N_Class; j++){*/
/*        max_val[j][i] = MINVAL;*/
/*        min_val[j][i] = MAXVAL;*/
/*    }*/
...
/* -------------------------------------------------------------------------------------- */
/* calc_vals() calcula los valores maximos y minimos de cada feature para dividir el histograma */
/* -------------------------------------------------------------------------------------- */
void calc_vals(){
  int i,j,c; //j feature
  double temp;
  ...
  for (i = 0; i < PTOT ; i++){
    for (j = 0 ; j < N_IN ; j++){
      c = data[i][N_IN]; //clase
      temp = data[i][j]; //value
      //printf("\n clase: %d,feature: %d  MaxVal: %f max_val actual: %f",c,j,temp,max_val[c][j]);
      if(temp >= max_val[c][j]){
        max_val[c][j]= temp;
      }
      if(temp <= min_val[c][j]){
  ...
  for (i =0 ; i<N_Class;i++){
    for(j=0 ; j< N_IN ; j++){
    printf("\nclase: %d,feature: %d  MaxVal: %f ",i,j,max_val[i][j]);
    printf("\nclase: %d,feature: %d  MinVal: %f ",i,j,min_val[i][j]);
    }
  }
