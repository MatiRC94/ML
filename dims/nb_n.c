...
/*ALLOCAR ESPACIO PARA LAS MATRICES DEL ALGORITMO*/
  ...
  for(i=0 ; i<N_Class; i++){
    mus[i]=calloc(N_IN,sizeof(double));
    sigmas[i]=calloc(N_IN,sizeof(double));
	  if(mus[i]==NULL || sigmas[i]==NULL) return 1;
...
double prob(double x,int feature,int clase)  {
  ...
  /*IMPLEMENTAR*/
  double temp_mu = mus[clase][feature] ;
  double temp_sigma = sqrt(sigmas[clase][feature]);
  
  double right =  exp(- ((x-temp_mu) * (x-temp_mu)) / (2 * temp_sigma * temp_sigma) );
  
  return (1/(temp_sigma * sqrt(2 * PI) )) * right;
...
int train(char *filename){
  ...
  for(i=0 ; i<N_TOTAL ; i++){
    for(j=0 ; j<N_IN ; j++){
          c = (int)data[i][N_IN]; //CLASE
          mus[c][j] = mus[c][j] + ( data[i][j] /(double)temp_cant_clases[c]);   //MEDIA
    }
  }
  ...
  for(i=0 ; i<N_TOTAL ; i++){
      for(j=0 ; j<N_IN ; j++){
        c = (int)data[i][N_IN]; //clase
        sigmas[c][j]= sigmas[c][j] + ( (data[i][j]-mus[c][j]) * (data[i][j]-mus[c][j]) ) / ((double)temp_cant_clases[c] - 1); // Varianza
      }
  }
  ...
  for(i=0 ; i< N_Class; i++){
    ...
    for(j=0 ; j< N_IN ; j++){
      printf("\nClase %d: %d patrones , en %d : mu: %f , sigma: %f, cantidad total elementos: %d, probabilidad de la clase: %f ",i,N_TOTAL,j,mus[i][j],sigmas[i][j],temp_cant_clases[i] ,prob_d[i] );
    }
  }
